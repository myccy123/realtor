...
def make_articles_data(thumbid,author,title,url,digest=""):
    data = """
        {
             "thumb_media_id":"%s",
             "author":"%s",
             ...
             "content":"<img src=\'http://mmbiz.qpic.cn/mmbiz_png/DMUsNpn7YxpJJjuFzdfYbz15GHpM0uORjv2zo6V3XetPuGCTHZhRUcsOazYdAtMI5Carr8DeeLFUM5CTI3rPzQ/0\'>",
             "digest":"%s",
             "show_cover_pic":0
         }
    """ % (thumbid,author,title,url,digest)
    return data
#http://mmbiz.qpic.cn/mmbiz_gif/DMUsNpn7Yxqv0u9FAWtSzVbscl63L4etPHHVEsEoW9ZMmpqgG0XPTsWlM21Xkc1iaIqDsYWaNKMmIEcJ8GgyT6A/0
#http://mmbiz.qpic.cn/mmbiz_png/DMUsNpn7YxpJJjuFzdfYbz15GHpM0uORjv2zo6V3XetPuGCTHZhRUcsOazYdAtMI5Carr8DeeLFUM5CTI3rPzQ/0


def make_sp():
    mls = 'r2177295'
    imgpath = '/root/myweb/data/listings/'+'%s_1.jpg' % mls
    thumbid = upload_material(imgpath)
    sql = """select price ,listingname ,cityname ,intro_zh ,url from app.singlepage_listing_for_sp where listingid = '%s' """ % mls
    row = MySql.sel_table(sql).next()
    data = """
        {
           "articles": [
                 %s
    ...
    """ % make_articles_data(thumbid, 'Webmainland' , '%s | %s' % (row[1],row[2]), row[4], '我的朋友向我推荐了这个海外房源推广活动，转发朋友圈截图反馈公众号，有机会获取房源推广现金奖励！')
